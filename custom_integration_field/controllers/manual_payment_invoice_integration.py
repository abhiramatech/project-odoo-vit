...
class ManualIntegrationPaymentInvoice(models.Model):
    _inherit = 'account.move'

    @api.model
    def Searching_PaymentInvoice(self, date_from, date_to):
        try:
    ...
    @api.model
    def Manual_Sync_PaymentInvoice_Integration(self, date_from, date_to):
        try:
            conn = pymssql.connect(server='pradovet.database.windows.net',
                                   port=1433,
                                   database='prado_odoo_prod', 
