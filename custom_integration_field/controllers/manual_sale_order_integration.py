...
class ManualIntegrationSalesOrder(models.Model):
    _inherit = 'sale.order'
    ...
    @api.model
    def Searching_InvoicesSales(self, date_from, date_to):
        try:
...
# --------------------------------------------------------------------------------------- #
        #                           MANUAL SALES ORDER                #
    def Manual_SalesOrder_Integration(self, date_from, date_to):
        try:
            conn = pymssql.connect(server='pradovet.database.windows.net',
                                   port=1433,
                                   database='prado_odoo_prod', 
