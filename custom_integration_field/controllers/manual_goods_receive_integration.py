...
class ManualIntegrationGoodsReceipt(models.Model):
    _inherit = 'stock.picking'

    def confirm_receipt_batch(self, batch, move_type):
    ...
    def Manual_Sync_GoodsReceipt_Integration(self, date_from, date_to):
        try:
            ...
            query = """
                ...
                FROM
                    t_gr_h h
                INNER JOIN
                    t_gr_d d ON h.doc_number = d.doc_number
                WHERE 
                    h.date_doc BETWEEN %s AND %s
                    AND h.sync_flag = 0
                    AND h.move_type = 'StockTake';
            """
            ...
            for doc_number, data_list in grouped_data.items():
                ...
                operation_type_search = self.env['stock.picking.type'].search([('name', '=', 'Goods Receipt')])
                location_id = self.env['stock.location'].search([('complete_name', '=', "Virtual Locations/Inventory adjustment")], limit=1)
                ...
                if unregistered_products:
                    ...
                    if not existing_error:
                        error_description = f"GR tidak bisa terbuat karena produk tidak terdaftar\n"
                        error_description += "\n".join([f"- {item_code}" for item_code in products_to_log])

                        log_error.create({
                            'vit_doc_type': 'Goods Receipt',
                        ...
                        insert_log_note_query = f"""
                            ...
                            VALUES ('Goods Receipt', '{doc_number}', '{date_doc}', 'Failed', '{error_description}', GETDATE(), GETDATE());
                        """
                    ...
                    for data in registered_products:
                        ...
                        source_loc = self.env['stock.location'].search([('complete_name', '=', "WH/Stock")], limit=1)

                        # Add lines to the Goods Receipt
                        product = self.env['product.product'].search([('default_code', '=', item_code)], limit=1)
                        
                        if product:
                            insert_t_gr_d_log_query = f"""
                                INSERT INTO t_gr_d_log (doc_number, line_number, item_code, qty, date_insert, date_sync, sync_flag, sync_desc)
                                VALUES ('{doc_num_detail}', '{line_number_detail}', '{item_code}', '{qty}', '{date_insert}', GETDATE(), 1, NULL);
                            """
                            cursor.execute(insert_t_gr_d_log_query)
                            conn.commit()
                    ...
                    source_loc_form = self.env['stock.location'].search([('complete_name', '=', "WH/Stock")], limit=1)    
                    ...
                    insert_t_gr_h_log_query = f"""
                        INSERT INTO t_gr_h_log (doc_number, move_type, date_doc, date_insert, date_sync, sync_flag, sync_desc)
                        VALUES ('{doc_number}', '{move_type}', '{date_doc}', '{date_insert}', GETDATE(), 1, NULL);
                    """
                    cursor.execute(insert_t_gr_h_log_query)
                    conn.commit()
                    ...
                    if order_count >= batch_size:
                        self.confirm_receipt_batch(batch, move_type)
                        batch = []
    ...
    @api.model
    def Searching_GoodsReceipt(self, date_from, date_to):
        try:
            conn = pymssql.connect(server='pradovet.database.windows.net',
                                port=1433,
                                database='prado_odoo_prod', 
