...
class ManualIntegrationCreditNote(models.Model):
    _inherit = 'account.move'

    def confirm_credit_note_batch(self, batch):
        for receipt in batch:
            receipt.action_post()

    @api.model
    def Searching_CreditNote(self, date_from, date_to):
        try:
    ...
    @api.model
    def Manual_Sync_CreditNote_Integration(self, date_from, date_to):
        try:
            conn = pymssql.connect(server='pradovet.database.windows.net',
                                   port=1433,
                                   database='prado_odoo_prod',
