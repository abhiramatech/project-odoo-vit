...
class ManualIntegrationReturnOrder(models.Model):
    ...
    @api.model
    def Manual_ReturnOrder_Integration(self, date_from, date_to):
        try:
    ...
    @api.model
    def Searching_ReturnOrder(self, date_from, date_to):
        try:
            conn = pymssql.connect(server='pradovet.database.windows.net',
                                port=1433,
                                database='prado_odoo_prod', 
            ...
            query = """
                ...
                WHERE 
                    ...
                    AND (h.sync_flag = 1 OR h.sync_flag IS NULL);
            """
            cursor.execute(query, (date_from, date_to))
            sql_data = cursor.fetchall()
