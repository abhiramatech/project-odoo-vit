...
class ManualIntegrationPurchaseOrder(models.Model):
    _inherit = 'purchase.order'

    def confirm_purchase_order_batch(self, batch, date_doc):
        for order in batch:
            doc_number = order.vit_trxid
            ...
            # Now, update the is_integrated field for related stock.picking records
            for picking in order.picking_ids.filtered(lambda picking: picking.state == 'assigned' and picking.picking_type_id.name == "Goods Receipt PO"):
                picking.write({'vit_trxid': doc_number})
                picking.button_validate()

    @api.model
    def Searching_PurchaseOrder(self, date_from, date_to):
        try:
    ...
    def Manual_Sync_PurchaseOrder_Integration(self, date_from, date_to):
        try:
            conn = pymssql.connect(server='pradovet.database.windows.net',
                                   port=1433,
                                   database='prado_odoo_prod', 
